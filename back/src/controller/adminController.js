...
exports.validateUser = async (req, res) => {
    try {
        ...
        if (user.status === "approved") {
            return res.status(400).json({message: "L'utilisateur est déjà approuvé"});
        }
...
exports.getAllUsersPendingStatus = async (req, res) => {
    try {
        const pendingUsers = await User.find({status: "pending"}, "id username email motivation createdAt");

        if (pendingUsers.length === 0) {
            return res.status(404).json({message: "Aucun utilisateur en attente de validation."});
        }
